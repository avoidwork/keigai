...
(function (global) {
   ...
   */
    ...
    */
			queryString: function (arg, qstring) {
				...
				if (result !== null && !string.isEmpty(result)) {
					result = result.split("&");
					array.iterate(result, function (prop) {
						var item = prop.split("=");
   ...
   */
		var xhr = function () {
			...
			var headers = {
				"user-agent": "keigai/1.2.2 node.js/" + process.versions.node.replace(/^v/, "") + " (" + string.capitalize(process.platform) + " V8/" + process.versions.v8 + " )",
