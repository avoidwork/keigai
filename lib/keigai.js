  ...
  var lib = (function () {
      ...
      _prototypeProperties(Observable, null, {
        ...
        hook: {
          ...
          value: function hook(target, ev) {
            ...
            this.hooks.set(target, function (arg) {
              ...
              if (!string.isEmpty(val)) {
                utility.iterate(self.filters, function (v, k) {
                  ...
                  array.each(queries, function (i, idx) {
                    _this4[idx] = "^.*" + string.escape(i).replace(/(^\*|\*$)/g, "").replace(/\*/g, ".*") + ".*";
                  });

                  _this4[k] = queries.join(",");
                });
    ...
    var list = {
      ...
      factory: function (target, store, template, options) {
        var ref = [store];
        var obj = new DataList(element.create("ul", { "class": "list" }, target), ref[0], template);
