...
/*
 __          .__             .__   ____     ________      ____________  
...
(function (global) {
   ...
   */
    ...
    */
							...
							if (this.patch) {
								if (type === "del") {
									array.each(data, function (i) {
										patch = patch.concat(_this18.del(i, false, true));
									});
								} else {
									array.each(data, function (i) {
										patch = patch.concat(_this18.set(i[_this18.key] || null, i, true, false, true));
									});
								}
     ...
     */
				value: function createPatch(overwrite, data, ogdata) {
					var _this19 = this;
					...
					if (overwrite) {
						array.each(array.keys(ogdata), function (k) {
							if (k !== _this19.key && data[k] === undefined) {
					...
					utility.iterate(data, function (v, k) {
						if (k !== _this19.key && ogdata[k] === undefined) {
     ...
     */
				value: function del(record) {
					...
					var jsonpatch = arguments[3] === undefined ? false : arguments[3];
					...
					if (record === undefined) {
						...
						if (this.uri === null || this.callback !== null) {
							...
							client.request(this.buildUri(record.key), "DELETE", null, utility.merge({ withCredentials: this.credentials }, this.headers)).then(function () {
								_this20.delComplete(record, reindex, batch, defer);
							}, function (e) {
								_this20.dispatch("failedDelete", e);
								defer.reject(e);
							});
						}
					}
					...
					if (!batch) {
						...
						if (reindex) {
							...
							array.each(record.indexes, function (i) {
								array.remove(_this21.indexes[i[0]][i[1]], record.index);

								if (_this21.indexes[i[0]][i[1]].length === 0) {
									delete _this21.indexes[i[0]][i[1]];
								}
							});
						}
					...
					if (custom) {
						fn = function (i) {
							...
							array.each(fields, function (f) {
								record[f] = f === _this22.key ? isNaN(i.key) ? i.key : Number(i.key) : utility.clone(i.data[f], true);
							});
						...
						fn = function (i) {
							...
							if (key) {
								record[_this22.key] = isNaN(i.key) ? i.key : Number(i.key);
							}
					...
					if (type === "undefined") {
						...
						if (record.indexOf(",") === -1) {
							...
							result = string.explode(record).map(function (i) {
								if (!isNaN(i)) {
									return _this23.records[parseInt(i, 10)];
								} else {
									return _this23.records[_this23.indexes.key[i]];
								}
							});
						}
					} else if (type === "number") {
				...
				value: (function (_join) {
					...
					if (join === "inner") {
						...
						fn = function (i) {
							...
							_this24.select(where).then(function (match) {
								if (match.length > 2) {
									defer.reject(new Error(label.databaseMoreThanOne));
								} else if (match.length === 1) {
									results.push(utility.merge(record, match[0].data));
					...
					if (this.total > 0) {
						array.each(this.records, function (record) {
							if (record !== undefined) {
								tmp[++i] = record;
								record.index = i;
								_this25.indexes.key[record.key] = i;
								_this25.setIndexes(record);
							}
						});
					}
     ...
     */
				value: function set(key, data) {
					...
					var jsonpatch = arguments[4] === undefined ? false : arguments[4];
					...
					if (typeof data === "string") {
						...
						if (string.isEmpty(key)) {
							...
							client.request(uri, "GET", null, utility.merge({ withCredentials: this.credentials }, this.headers)).then(function (arg) {
								_this27.setComplete(record, key, _this27.source ? utility.walk(arg, _this27.source) : arg, batch, overwrite, defer);
							}, function (e) {
								_this27.dispatch("failedSet", e);
								defer.reject(e);
							});
						}
					} else {
						...
						if (batch || this.uri === null) {
							...
							rdefer.then(function (arg) {
								...
								if (rdefer.xhr.status !== 204 && rdefer.xhr.status < 300) {
									change = key === null ? _this27.source ? utility.walk(arg, _this27.source) : arg : odata;
								} else {
									change = odata;
								}

								_this27.setComplete(record, key, change, batch, overwrite, defer);
							}, function (e) {
								if (method === "PATCH") {
									method = "PUT";
									...
									client.request(uri, method, data, utility.merge({ withCredentials: _this27.credentials }, _this27.headers)).then(function () {
										_this27.setComplete(record, key, odata, batch, overwrite, defer);
									}, function (e) {
										_this27.dispatch("failedSet", e);
										defer.reject(e);
									});
     ...
     */
				value: function setExpires(arg) {
					...
					utility.repeat(function () {
						if (_this28.uri === null) {
							_this28.setExpires(null);

							return false;
						}

						_this28.dispatch("beforeExpire");
						cache.expire(_this28.uri);
						_this28.dispatch("expire");
						_this28.dispatch("afterExpire");
					}, expires, id, false);
				}
			}, {
				key: "setIndexes",
     ...
     */
				value: function setIndexes(arg) {
					...
					array.each(this.index, function (i) {
						...
						if (_this29.indexes[i] === undefined) {
							_this29.indexes[i] = {};
						}
						...
						if (_this29.indexes[i][values] === undefined) {
							_this29.indexes[i][values] = [];
						}

						if (!array.contains(_this29.indexes[i][values], arg.index)) {
							_this29.indexes[i][values].push(arg.index);
     ...
     */
				value: function sort(query, create, where) {
					...
					var next = function next(records) {
						...
						if (_this31.total === 0) {
							defer.resolve([]);
						} else if (!create && _this31.views[view]) {
							defer.resolve(_this31.views[view]);
						} else if (webWorker) {
							defer.then(function (arg) {
								_this31.views[view] = arg;

								return _this31.views[view];
							}, function (e) {
								utility.error(e);
							});
							...
							try {
								worker = utility.worker(defer);
								worker.postMessage({ cmd: "sort", indexes: _this31.indexes, records: records, query: query });
							} catch (e) {
								// Probably IE10, which doesn't have the correct security flag for local loading
								webWorker = false;

								_this31.views[view] = array.keySort(records, query, "data");
								defer.resolve(_this31.views[view]);
							}
						} else {
							_this31.views[view] = array.keySort(records, query, "data");
					...
					if (!regex.number_string_object.test(typeof obj) || !regex.get_remove_set.test(op)) {
						...
						if (mongo) {
							mongodb.connect(this.mongodb, function (e, db) {
								...
								db.collection(_this32.id, function (e, collection) {
									if (e) {
										db.close();
										return defer.reject(e);
									}
										...
										if (record) {
											collection.find({ _id: obj.key }).limit(1).toArray(function (e, recs) {
												...
												if (e) {
													...
													_this32.set(key, recs[0], true).then(function (rec) {
														defer.resolve(rec);
													}, function (e) {
														defer.reject(e);
													});
											...
											collection.find({}).toArray(function (e, recs) {
												...
												if (nth > 0) {
													_this32.records = recs.map(function (r) {
														var rec = { key: r._id, index: ++i, data: {} };

														_this32.indexes.key[rec.key] = rec.index;
														rec.data = r;
														delete rec.data._id;
														_this32.setIndexes(rec);
										...
										if (record) {
											...
											collection.remove({}, { w: 1, safe: true }, function (e) {
												...
												if (e) {
													...
													array.each(_this32.records, function (i) {
														var data = {};
														var defer2 = deferred();
     ...
     */
      ...
      */
					var success = function success(arg) {
						...
						if (_this33.source !== null) {
							arg = utility.walk(arg, _this33.source);
						}
						...
						_this33.batch("set", data, true).then(function (arg) {
							if (events) {
								_this33.dispatch("afterSync", arg);
							}
   ...
   */
    ...
    */
			banner: function banner() {
				console.log([" __          .__             .__   ____     ________      ____________  ", "|  | __ ____ |__| _________  |__| /_   |    \\_____  \\    /_   \\_____  \\ ", "|  |/ // __ \\|  |/ ___\\__  \\ |  |  |   |      _(__  <     |   | _(__  < ", "|    <\\  ___/|  / /_/  > __ \\|  |  |   |     /       \\    |   |/       \\", "|__|_ \\\\___  >__\\___  (____  /__|  |___| /\\ /______  / /\\ |___/______  /", "     \\/    \\/  /_____/     \\/            \\/        \\/  \\/            \\/ "].join("\n"));
			},
   ...
   */
		var xhr = function xhr() {
			...
			var headers = {
				"user-agent": "keigai/1.3.13 node.js/" + process.versions.node.replace(/^v/, "") + " (" + string.capitalize(process.platform) + " V8/" + process.versions.v8 + " )",
