...
(function (global) {
      ...
      */
					...
					value: function refresh() {
						...
						next = function (args) {
							...
							if (filter) {
								array.each(_this.records, function (i) {
							...
							array.each(_this.current, function (i) {
								var html = fn(i);
								var hash = btoa(html);
    ...
    */
			...
			function Deferred() {
				...
				this.promise.then(function (arg) {
					array.each(_this.onDone, function (i) {
						i(arg);
					});

					array.each(_this.onAlways, function (i) {
						i(arg);
					});
					...
					array.each(_this.onFail, function (i) {
						i(arg);
					});
						...
						if (!regex.set_del.test(type) || sync && regex.del.test(type) || typeof data !== "object") {
							...
							if (data.length === 0) {
								...
								utility.when(deferreds).then(function (args) {
									...
									array.each(_this.lists, function (i) {
										i.refresh(true);
									});
   ...
   */
		var xhr = function () {
			...
			var headers = {
				"user-agent": "keigai/1.3.4 node.js/" + process.versions.node.replace(/^v/, "") + " (" + string.capitalize(process.platform) + " V8/" + process.versions.v8 + " )",
