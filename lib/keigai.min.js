...
e);defer.reject(e)})}return defer};DataStore.prototype.delComplete=function(record,reindex,batch,defer){var self=this;delete this.indexes.key[record.key];delete this.versions[record.key];array.remove(this.records,record.index);this.total--;this.views={};if(!batch){if(reindex)this.reindex();else array.each(record.indexes,function(i){array.remove(self.indexes[i[0]][i[1]],record.index);if(self.indexes[i[0]][i[1]].length===0)delete self.indexes[i[0]][i[1]]});if(this.autosave)this.purge(record.key);if(this.events)this.dispatch("afterDelete",
record);array.each(this.lists,function(i){i.refresh()})}return defer.resolve(record.key)};DataStore.prototype.dump=function(args,fields){args=args||this.records;var self=this,custom=fields instanceof Array&&fields.length>0,key=this.key!==null,fn;if(custom)fn=function(i){var record={};array.each(fields,function(f){record[f]=f===self.key?i.key:utility.clone(i.data[f],true)});return record};else fn=function(i){var record={};if(key)record[self.key]=i.key;utility.iterate(i.data,function(v,k){record[k]=
utility.clone(v,true)});return record};return args.map(fn)};DataStore.prototype.get=function(record,offset){var type=typeof record,self=this,result;if(type==="undefined")result=this.records;else if(type==="string")if(record.indexOf(",")===-1)result=this.records[this.indexes.key[record]];else result=string.explode(record).map(function(i){if(!isNaN(i))return self.records[parseInt(i,10)];else return self.records[self.indexes.key[i]]});else if(type==="number")if(isNaN(offset))result=this.records[parseInt(record,
10)];else result=array.limit(this.records,parseInt(record,10),parseInt(offset,10));return result},DataStore.prototype.join=function(arg,field,join){join=join||"inner";var self=this,defer=deferred.factory(),results=[],deferreds=[],key=field===this.key,keys=array.merge(array.cast(this.records[0].data,true),array.cast(arg.records[0].data,true)),fn;if(join==="inner")fn=function(i){var where={},record=utility.clone(i.data,true),defer=deferred.factory();where[field]=key?i.key:record[field];arg.select(where).then(function(match){if(match.length>
