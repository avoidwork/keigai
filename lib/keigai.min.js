...
DataStore.prototype.buildUri=function(key){var parsed=utility.parse(this.uri);return parsed.protocol+"//"+parsed.host+parsed.pathname+(regex.endslash.test(parsed.pathname)?"":"/")+key};DataStore.prototype.clear=function(sync){sync=sync===true;var events=this.events===true,resave=this.autosave===true;if(!sync){if(events)this.dispatch("beforeClear");array.each(this.lists,function(i){i.teardown(true)});this.autosave=false;this.callback=null;this.credentials=null;this.lists=[];this.events=true;this.expires=
null;this.headers={Accept:"application/json"};this.ignore=[];this.index=[];this.indexes={key:{}};this.key=null;this.loaded=false;this.records=[];this.source=null;this.total=0;this.versions={};this.versioning=true;this.views={};this.uri=null;if(events)this.dispatch("afterClear")}else{this.indexes={key:{}};this.loaded=false;this.records=[];this.total=0;this.views={};array.each(this.lists,function(i){i.refresh()})}if(resave)this.save();return this};DataStore.prototype.del=function(record,reindex,batch){record=
record.key?record:this.get(record);reindex=reindex!==false;batch=batch===true;var self=this,defer=deferred.factory();if(record===undefined)defer.reject(new Error(label.invalidArguments));else{if(this.events)self.dispatch("beforeDelete",record);if(this.uri===null||this.callback!==null)this.delComplete(record,reindex,batch,defer);else client.request(this.buildUri(record.key),"DELETE",null,utility.merge({withCredentials:this.credentials},this.headers)).then(function(){self.delComplete(record,reindex,
batch,defer)},function(e){self.dispatch("failedDelete",e);defer.reject(e)})}return defer};DataStore.prototype.delComplete=function(record,reindex,batch,defer){var self=this;delete this.indexes.key[record.key];delete this.versions[record.key];this.total--;this.views={};if(!batch){array.remove(this.records,record.index);if(reindex)this.reindex();else array.each(record.indexes,function(i){array.remove(self.indexes[i[0]][i[1]],record.index);if(self.indexes[i[0]][i[1]].length===0)delete self.indexes[i[0]][i[1]]});
