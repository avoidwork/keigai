...
if(this.filter&&this.filtered.length===0||this.total===0||total===1)return this;if(start<1){diff=number.diff(start,1);start=start+diff;end=end+diff}if(end>total){end=total;start=end-range+1;if(start<1)start=1}if(number.diff(start,end)>=range)--end;array.each(string.explode(pos),function(i){var current=false,more=page>1,next=page+1<=total,last=page>=total,el,n;el=element.create("ul",{"class":"list pages hidden "+i,id:obj.id+"-pages-"+i},obj,i==="bottom"?"after":"before");element.create(more?"a":"span",
{"class":"first page","data-page":1,innerHTML:"&lt;&lt;"},element.create("li",{},el));element.create(more?"a":"span",{"class":"prev page","data-page":page-1,innerHTML:"&lt;"},element.create("li",{},el));for(n=start;n<=end;n++){current=n===page;element.create(current?"span":"a",{"class":current?"current page":"page","data-page":n,innerHTML:n},element.create("li",{},el))}element.create(next?"a":"span",{"class":"next page","data-page":next?page+1:null,innerHTML:"&gt;"},element.create("li",{},el));element.create(last?
"span":"a",{"class":"last page","data-page":last?null:total,innerHTML:"&gt;&gt;"},element.create("li",{},el));element.klass(el,"hidden",false);self.observer.hook(el,"click")});return this};DataList.prototype.refresh=function(create){var self=this,el=this.element,template=typeof this.template=="object",filter=this.filter!==null,items=[],callback=typeof this.callback=="function",reg=new RegExp,registry=[],range=[],fn,ceiling,next;create=create===true;this.dispatch("beforeRefresh",el);if(!template)fn=
function(i){var html=self.template,items=array.unique(html.match(/\{\{[\w\.\-\[\]]+\}\}/g));html=html.replace("{{"+self.store.key+"}}",i.key);array.each(items,function(attr){var key=attr.replace(/\{\{|\}\}/g,""),value=utility.walk(i.data,key);if(value===undefined)value="";reg.compile(string.escape(attr),"g");html=html.replace(reg,value)});html=html.replace(/\{\{.*\}\}/g,self.placeholder);return'<li data-key="'+i.key+'">'+html+"</li>"};else fn=function(i){var obj=json.encode(self.template),items=array.unique(obj.match(/\{\{[\w\.\-\[\]]+\}\}/g));
obj=obj.replace("{{"+self.store.key+"}}",i.key);array.each(items,function(attr){var key=attr.replace(/\{\{|\}\}/g,""),value=utility.walk(i.data,key)||"";reg.compile(string.escape(attr),"g");obj=obj.replace(reg,json.encode(value).replace(/(^")|("$)/g,""))});obj=json.decode(obj.replace(/\{\{.*\}\}/g,self.placeholder));return{li:obj}};next=function(args){self.records=args;self.total=self.records.length;self.filtered=[];self.current=[];if(filter)array.each(self.records,function(i){utility.iterate(self.filter,
function(v,k){var reg,key;if(array.contains(registry,i.key))return false;v=string.explode(v);reg=new RegExp,key=k===self.store.key;array.each(v,function(query){var value=!key?utility.walk(i.data,k):"";utility.compile(reg,query,"i");if(key&&reg.test(i.key)||reg.test(value)){registry.push(i.key);self.filtered.push(i);return false}})})});if(self.pageSize!==null&&!isNaN(self.pageIndex)&&!isNaN(self.pageSize)){ceiling=list.pages.call(self);if(ceiling>0&&self.pageIndex>ceiling)return self.page(ceiling);
else if(self.total>0){range=list.range.call(self);self.current=array.limit(!filter?self.records:self.filtered,range[0],range[1])}}else self.current=!filter?self.records:self.filtered;array.each(self.current,function(i){var html=fn(i),hash=btoa(html);items.push({key:i.key,template:html,hash:hash})});utility.render(function(){var destroy=[],callbacks=[],i,nth;if(items.length===0)element.html(el,'<li class="empty">'+self.emptyMsg+"</li>");else if(self.items.length===0){element.html(el,items.map(function(i){return i.template}).join(""));
if(callback)array.each(array.cast(el.childNodes),function(i){self.callback(i)})}else{array.each(items,function(i,idx){if(self.items[idx]!==undefined&&self.items[idx].hash!==i.hash){element.data(element.html(el.childNodes[idx],i.template.replace(/<li data-key=\"\d+\">|<\/li>/g,"")),"key",i.key);callbacks.push(idx)}else if(self.items[idx]===undefined){element.create(i.template,null,el);callbacks.push(idx)}});if(items.length<self.items.length){i=items.length-1;nth=self.items.length;while(++i<nth)destroy.push(i);
array.each(destroy.reverse(),function(i){element.destroy(el.childNodes[i])})}if(callback)array.each(callbacks,function(i){self.callback(el.childNodes[i])})}self.items=items;if(self.pageSize!==null&&regex.top_bottom.test(self.pagination)&&!isNaN(self.pageIndex)&&!isNaN(self.pageSize))self.pages();else array.each(utility.$("#"+el.id+"-pages-top, #"+el.id+"-pages-bottom"),function(i){element.destroy(i)});self.dispatch("afterRefresh",el)})};if(this.where===null)string.isEmpty(this.order)?next(this.store.get()):
this.store.sort(this.order,create).then(next,function(e){utility.error(e);self.dispatch("error",e)});else if(string.isEmpty(this.order))this.store.select(this.where).then(next,function(e){utility.error(e);self.dispatch("error",e)});else this.store.sort(this.order,create,this.where).then(next,function(e){utility.error(e);self.dispatch("error",e)});return this};DataList.prototype.remove=function(record){var self=this;this.store.del(record).then(null,function(e){utility.error(e);self.dispatch("error",
e)});return this};DataList.prototype.sort=function(order){this.order=order;return this.refresh()};DataList.prototype.teardown=function(destroy){destroy=destroy===true;var self=this;array.each(this.store.lists,function(i,idx){if(i.id===self.id){this.remove(idx);return false}});delete this.observer.hooks[this.element.id];if(destroy){element.destroy(this.element);this.element=null}return this};DataList.prototype.update=function(key,data){var self=this;this.store.update(key,data).then(null,function(e){utility.error(e);
self.dispatch("error",e)});return this};var deferred={factory:function(){return new Deferred}};function Deferred(){var self=this;this.promise=promise.factory();this.onDone=[];this.onAlways=[];this.onFail=[];this.promise.then(function(arg){array.each(self.onDone,function(i){i(arg)});array.each(self.onAlways,function(i){i(arg)});self.onAlways=[];self.onDone=[];self.onFail=[]},function(arg){array.each(self.onFail,function(i){i(arg)});array.each(self.onAlways,function(i){i(arg)});self.onAlways=[];self.onDone=
[];self.onFail=[]})}Deferred.prototype.constructor=Deferred;Deferred.prototype.always=function(arg){this.onAlways.push(arg);return this};Deferred.prototype.done=function(arg){this.onDone.push(arg);return this};Deferred.prototype.fail=function(arg){this.onFail.push(arg);return this};Deferred.prototype.reject=function(arg){this.promise.reject.call(this.promise,arg);return this};Deferred.prototype.resolve=function(arg){this.promise.resolve.call(this.promise,arg);return this};Deferred.prototype.then=
...
version:"0.6.2"}}();if(typeof exports!="undefined")module.exports=lib;else if(typeof define=="function")define(function(){return lib});else global.keigai=lib})(this);
