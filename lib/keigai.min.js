...
i,offset;if(async!==true){i=-1;while(++i<nth)if(fn.call(obj,obj[i],i)===false)break}else{size=size||10;offset=0;if(size>nth)size=nth;utility.repeat(function(){var i=-1,idx;while(++i<size){idx=i+offset;if(idx===nth||fn.call(obj,obj[idx],idx)===false)return false}offset+=size;if(offset>=nth)return false},undefined,undefined,false)}return obj},empty:function(obj){return obj.length===0},equal:function(obj1,obj2){return json.encode(obj1)===json.encode(obj2)},fib:function(arg){var result=[1,1],first=result[0],
second=result[1],sum;arg=(arg||100)-1;if(isNaN(arg)||arg<2)throw new Error(label.invalidArguments);while(--arg){sum=first+second;first=second;second=sum;result.push(sum)}return result},fill:function(obj,arg,start,offset){var fn=typeof arg=="function",l=obj.length,i=!isNaN(start)?start:0,nth=!isNaN(offset)?i+offset:l-1;if(nth>l-1)nth=l-1;while(i<=nth){obj[i]=fn?arg(obj[i]):arg;i++}return obj},first:function(obj){return obj[0]},flat:function(obj){var result=[];result=obj.reduce(function(a,b){return a.concat(b)},
result);return result},forEach:function(obj,fn,async,size){return array.each(obj,fn,async,size)},fromObject:function(obj){return array.mingle(array.keys(obj),array.cast(obj))},index:function(obj,arg){return obj.indexOf(arg)},indexed:function(obj){var indexed=[];utility.iterate(obj,function(v){indexed.push(v)});return indexed},intersect:function(obj1,obj2){var a=obj1.length>obj2.length?obj1:obj2,b=a===obj1?obj2:obj1;return a.filter(function(key){return array.contains(b,key)})},keepIf:function(obj,
fn){if(typeof fn!="function")throw new Error(label.invalidArguments);var result=[],remove=[];result=obj.filter(fn);remove=array.diff(obj,result);array.each(remove,function(i){array.remove(obj,array.index(obj,i))});return obj},keys:function(obj){return Object.keys(obj)},keySort:function(obj,query,sub){query=query.replace(/\s*asc/ig,"").replace(/\s*desc/ig," desc");var queries=string.explode(query).map(function(i){return i.split(" ")}),sorts=[],braceS='["',braceE='"]';if(sub&&sub!=="")sub="."+sub;else sub=
"";array.each(queries,function(i){var s=".",e="";if(regex.not_dotnotation.test(i[0])){s=braceS;e=braceE}sorts.push("try {");if(i[1]==="desc"){sorts.push("if ( a"+sub+s+i[0]+e+" < b"+sub+s+i[0]+e+" ) return 1;");sorts.push("if ( a"+sub+s+i[0]+e+" > b"+sub+s+i[0]+e+" ) return -1;")}else{sorts.push("if ( a"+sub+s+i[0]+e+" < b"+sub+s+i[0]+e+" ) return -1;");sorts.push("if ( a"+sub+s+i[0]+e+" > b"+sub+s+i[0]+e+" ) return 1;")}sorts.push("} catch (e) {");sorts.push("try {");sorts.push("if ( a"+sub+s+i[0]+
e+" !== undefined ) return "+(i[1]==="desc"?"-1":"1")+";");sorts.push("} catch (e) {}");sorts.push("try {");sorts.push("if ( b"+sub+s+i[0]+e+" !== undefined ) return "+(i[1]==="desc"?"1":"-1")+";");sorts.push("} catch (e) {}");sorts.push("}")});sorts.push("return 0;");return obj.sort(new Function("a","b",sorts.join("\n")))},last:function(obj,arg){var n=obj.length-1;if(arg>=n+1)return obj;else if(isNaN(arg)||arg===1)return obj[n];else return array.limit(obj,n- --arg,n)},limit:function(obj,start,offset){var result=
[],i=start-1,nth=start+offset,max=obj.length;if(max>0)while(++i<nth&&i<max)result.push(obj[i]);return result},max:function(obj){return array.last(obj.sort(array.sort))},mean:function(obj){return obj.length>0?array.sum(obj)/obj.length:undefined},median:function(obj){var nth=obj.length,mid=number.round(nth/2,"down"),sorted=obj.sort(array.sort);return number.odd(nth)?sorted[mid]:(sorted[mid-1]+sorted[mid])/2},merge:function(obj1,obj2){array.each(obj2,function(i){array.add(obj1,i)});return obj1},min:function(obj){return obj.sort(array.sort)[0]},
mingle:function(obj1,obj2){var result;result=obj1.map(function(i,idx){return[i,obj2[idx]]});return result},mode:function(obj){var values={},count=0,mode=[],nth,result;array.each(obj,function(i){if(!isNaN(values[i]))values[i]++;else values[i]=1});count=array.max(array.cast(values));utility.iterate(values,function(v,k){if(v===count)mode.push(number.parse(k))});nth=mode.length;if(nth>0)result=nth===1?mode[0]:mode;return result},percents:function(obj,precision,total){var result=[],custom=false,last,padding,
sum;precision=precision||0;if(total===undefined)total=array.sum(obj);else custom=true;array.each(obj,function(i){result.push(number.parse((i/total*100).toFixed(precision)))});if(!custom){sum=array.sum(result);if(sum<100){padding=number.parse(number.diff(sum,100).toFixed(precision));last=array.last(result)+padding;result[result.length-1]=last}else if(sum>100){padding=number.parse(number.diff(sum,100).toFixed(precision));last=number.parse((array.last(result)-padding).toFixed(precision));result[result.length-
...
record);array.each(this.lists,function(i){i.refresh()})}else this.records[record.index]=null;return defer.resolve(record.key)};DataStore.prototype.dump=function(args,fields){args=args||this.records;var self=this,custom=fields instanceof Array&&fields.length>0,key=this.key!==null,fn;if(custom)fn=function(i){var record={};array.each(fields,function(f){record[f]=f===self.key?i.key:utility.clone(i.data[f],true)});return record};else fn=function(i){var record={};if(key)record[self.key]=i.key;utility.iterate(i.data,
function(v,k){record[k]=utility.clone(v,true)});return record};return args.map(fn)};DataStore.prototype.get=function(record,offset){var type=typeof record,self=this,result;if(type==="undefined")result=this.records;else if(type==="string")if(record.indexOf(",")===-1)result=this.records[this.indexes.key[record]];else result=string.explode(record).map(function(i){if(!isNaN(i))return self.records[parseInt(i,10)];else return self.records[self.indexes.key[i]]});else if(type==="number")if(isNaN(offset))result=
this.records[parseInt(record,10)];else result=array.limit(this.records,parseInt(record,10),parseInt(offset,10));return utility.clone(result,true)},DataStore.prototype.join=function(arg,field,join){join=join||"inner";var self=this,defer=deferred.factory(),results=[],deferreds=[],key=field===this.key,keys=array.merge(array.keys(this.records[0].data),array.keys(arg.records[0].data)),fn;if(join==="inner")fn=function(i){var where={},record=i.data,defer=deferred.factory();where[field]=key?i.key:record[field];
...
1){results.push(utility.merge(record,match[0].data));defer.resolve(true)}else{array.each(keys,function(i){if(record[i]===undefined)record[i]=null});results.push(record);defer.resolve(true)}});deferreds.push(defer)};else if(join==="right")fn=function(i){var where={},record=i.data,defer=deferred.factory();where[field]=key?i.key:record[field];self.select(where).then(function(match){if(match.length>2)defer.reject(new Error(label.databaseMoreThanOne));else if(match.length===1){results.push(utility.merge(record,
...
return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setIndexes=function(arg){var self=this,delimter="|";arg.indexes=[];array.each(this.index,function(i){var keys=i.split(delimter),values="";if(self.indexes[i]===undefined)self.indexes[i]={};array.each(keys,function(k,kdx){values+=(kdx>0?delimter:"")+arg.data[k]});if(self.indexes[i][values]===undefined)self.indexes[i][values]=[];self.indexes[i][values].push(arg.index);
