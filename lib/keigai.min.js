...
function(destroy){destroy=destroy===true;var self=this,id=this.element.id;array.each(this.store.lists,function(i,idx){if(i.id===self.id){array.remove(this,idx);return false}});delete this.observer.hooks[id];if(destroy){element.destroy(this.element);array.each(utility.$("#"+id+"-pages-top, #"+id+"-pages-bottom"),function(i){element.destroy(i)});this.element=null}return this};DataList.prototype.update=function(key,data){var self=this;this.store.update(key,data).then(null,function(e){utility.error(e);
...
key==="selected"){target=utility.dom("#"+obj.id+' option[selected="selected"]')[0];if(target){target.selected=false;target.removeAttribute("selected")}}else obj.removeAttribute(key);return obj},scrollTo:function(obj,ms,offsetTop,offsetLeft){var pos=array.remove(element.position(obj),2,3);if(!isNaN(offsetTop))pos[0]+=offsetTop;if(!isNaN(offsetLeft))pos[1]+=offsetLeft;return client.scroll(pos,ms)},serialize:function(obj,string,encode){string=string===true;encode=encode!==false;var registry={},children,
...
f===self.key?i.key:utility.clone(i.data[f],true)});return record};else fn=function(i){var record={};if(key)record[self.key]=i.key;utility.iterate(i.data,function(v,k){record[k]=utility.clone(v,true)});return record};return args.map(fn)};DataStore.prototype.get=function(record,offset){var type=typeof record,self=this,result;if(type==="undefined")result=this.records;else if(type==="string")if(record.indexOf(",")===-1)result=this.records[this.indexes.key[record]];else result=string.explode(record).map(function(i){if(!isNaN(i))return self.records[parseInt(i,
10)];else return self.records[self.indexes.key[i]]});else if(type==="number")if(isNaN(offset))result=this.records[parseInt(record,10)];else result=array.limit(this.records,parseInt(record,10),parseInt(offset,10));return utility.clone(result,true)},DataStore.prototype.join=function(arg,field,join){join=join||"inner";var self=this,defer=deferred.factory(),results=[],deferreds=[],key=field===this.key,keys=array.merge(array.keys(this.records[0].data),array.keys(arg.records[0].data)),fn;if(join==="inner")fn=
function(i){var where={},record=i.data,defer=deferred.factory();where[field]=key?i.key:record[field];arg.select(where).then(function(match){if(match.length>2)defer.reject(new Error(label.databaseMoreThanOne));else if(match.length===1){results.push(utility.merge(record,match[0].data));defer.resolve(true)}else defer.resolve(false)});deferreds.push(defer)};else if(join==="left")fn=function(i){var where={},record=i.data,defer=deferred.factory();where[field]=key?i.key:record[field];arg.select(where).then(function(match){if(match.length>
...
""),uri;if(typeof data=="string"){if(data.indexOf("//")===-1)if(data.charAt(0)!=="/")uri=this.buildUri(data);else if(self.uri!==null&&regex.root.test(data))uri=parsed.protocol+"//"+parsed.host+data;else uri=data;else uri=data;key=uri.replace(regex.not_endpoint,"");if(string.isEmpty(key))defer.reject(new Error(label.invalidArguments));else{if(!batch&&events)self.dispatch("beforeSet",{key:key,data:data});client.request(uri,"GET",null,utility.merge({withCredentials:self.credentials},self.headers)).then(function(arg){self.setComplete(record,
key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}else{if(!batch&&events)self.dispatch("beforeSet",{key:key,data:data});if(batch||this.uri===null)this.setComplete(record,key,data,batch,overwrite,defer);else{if(key!==null){method="PUT";uri=this.buildUri(key);if(client.allows(uri,"patch"))method="PATCH";else if(record!==null&&!overwrite)utility.iterate(record.data,function(v,k){data[k]=v})}else uri=this.uri;client.request(uri,
method,data,utility.merge({withCredentials:this.credentials},this.headers)).then(function(arg){self.setComplete(record,key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}return defer};DataStore.prototype.setComplete=function(record,key,data,batch,overwrite,defer){this.views={};if(!key)if(this.key!==null&&data[this.key])key=data[this.key].toString();else key=utility.uuid();if(this.key)delete data[this.key];if(record===
null){record={index:this.total++,key:key,data:data,indexes:[]};this.indexes.key[key]=record.index;this.records[record.index]=record;if(this.versioning){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}}else{if(this.versioning){if(this.versions[record.key]===undefined){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}this.versions[record.key].set("v"+ ++this.versions[record.key].nth,this.dump([record])[0])}if(overwrite)record.data={};utility.iterate(data,
function(v,k){record.data[k]=v})}this.setIndexes(record);if(!batch){if(this.autosave)this.save();if(this.events)this.dispatch("afterSet",record);array.each(this.lists,function(i){i.refresh()})}defer.resolve(record);return this};DataStore.prototype.setExpires=function(arg){var id=this.id+"Expire",expires=arg,self=this;if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.invalidArguments);if(this.expires===arg)return;this.expires=arg;utility.clearTimers(id);if(arg===
null)return;utility.repeat(function(){if(self.uri===null){self.setExpires(null);return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setIndexes=function(arg){var self=this,delimter="|";arg.indexes=[];array.each(this.index,function(i){var keys=i.split(delimter),values="";if(self.indexes[i]===undefined)self.indexes[i]={};array.each(keys,function(k,kdx){values+=(kdx>0?delimter:"")+arg.data[k]});
if(self.indexes[i][values]===undefined)self.indexes[i][values]=[];self.indexes[i][values].push(arg.index);arg.indexes.push([i,values])});return this};DataStore.prototype.setUri=function(arg){var defer=deferred.factory(),parsed;if(arg!==null&&string.isEmpty(arg))defer.reject(new Error(label.invalidArguments));if(arg===null)this.uri=arg;else{parsed=utility.parse(arg);this.uri=parsed.protocol+"//"+parsed.host+parsed.path;if(!string.isEmpty(parsed.auth)&&!this.headers.authorization&&!this.headers.Authorization)this.headers.Authorization=
"Basic "+btoa(decodeURIComponent(parsed.auth));this.on("expire",function(){this.sync()},"resync",this);cache.expire(this.uri);this.sync().then(function(arg){defer.resolve(arg)},function(e){defer.reject(e)})}return defer};DataStore.prototype.sort=function(query,create,where){create=create===true||where instanceof Object;var self=this,view=string.toCamelCase(string.explode(query).join(" ")),defer=deferred.factory(),next,worker;next=function(records){if(self.total===0)defer.resolve([]);else if(!create&&
self.views[view])defer.resolve(self.views[view]);else if(webWorker){defer.then(function(arg){self.views[view]=arg;return self.views[view]},function(e){utility.error(e)});try{worker=utility.worker(defer);worker.postMessage({cmd:"sort",indexes:self.indexes,records:records,query:query})}catch(e){webWorker=false;self.views[view]=array.keySort(records,query,"data");defer.resolve(self.views[view])}}else{self.views[view]=array.keySort(records,query,"data");defer.resolve(self.views[view])}};if(!where)next(utility.clone(this.records,
true));else this.select(where).then(next,function(e){defer.reject(e)});return defer};DataStore.prototype.storage=function(obj,op,type){var self=this,record=false,mongo=!string.isEmpty(this.mongodb),session=type==="session"&&typeof sessionStorage!="undefined",defer=deferred.factory(),data,key,result;if(!regex.number_string_object.test(typeof obj)||!regex.get_remove_set.test(op))defer.reject(new Error(label.invalidArguments));else{record=regex.number_string.test(typeof obj)||obj.hasOwnProperty("data");
if(op!=="remove"){if(record&&!(obj instanceof Object))obj=this.get(obj);key=record?obj.key:obj.id}else if(op==="remove"&&record)key=obj.key||obj;if(mongo)mongodb.connect(this.mongodb,function(e,db){if(e){if(db)db.close();return defer.reject(e)}db.collection(self.id,function(e,collection){if(e){db.close();return defer.reject(e)}if(op==="get")if(record)collection.find({_id:obj.key}).limit(1).toArray(function(e,recs){db.close();if(e)defer.reject(e);else if(recs.length===0)defer.resolve(null);else{delete recs[0]._id;
self.set(key,recs[0],true).then(function(rec){defer.resolve(rec)},function(e){defer.reject(e)})}});else collection.find({}).toArray(function(e,recs){var i,nth;if(e){db.close();return defer.reject(e)}i=-1;nth=recs.length;if(nth>0){self.records=recs.map(function(r){var rec={key:r._id,index:++i,data:{}};self.indexes.key[rec.key]=rec.index;rec.data=r;delete rec.data._id;self.setIndexes(rec);return rec});self.total=nth}db.close();defer.resolve(self.records)});else if(op==="remove")collection.remove(record?
...
"y").replace(/ses$/,"se").replace(/s$/,"")},toFunction:function(obj){var args=string.trim(obj.replace(/^.*\(/,"").replace(/[\t|\r|\n|\"|\']+/g,"").replace(/\).*/,"")),body=string.trim(obj.replace(/^.*\{/,"").replace(/\}$/,""));return Function.apply(Function,string.explode(args).concat([body]))},trim:function(obj){return obj.replace(/^(\s+|\t+|\n+)|(\s+|\t+|\n+)$/g,"")},unCamelCase:function(obj){return string.trim(obj.replace(/([A-Z])/g," $1").toLowerCase())},uncapitalize:function(obj){obj=string.trim(obj);
return obj.charAt(0).toLowerCase()+obj.slice(1)},unhyphenate:function(obj,caps){if(caps!==true)return string.explode(obj,"-").join(" ");else return string.explode(obj,"-").map(function(i){return string.capitalize(i)}).join(" ")}};var utility={timer:{},repeating:{},$:function(arg){var result;if(!arg);else if(regex.html.test(arg))result=[element.create(arg)];else{arg=string.trim(arg);if(arg.indexOf(",")===-1){result=utility.dom(arg);if(result){if(isNaN(result.length))result=[result]}else result=[]}else{result=
[];array.each(string.explode(arg),function(query){var obj=utility.dom(query);if(obj instanceof Array)result=result.concat(obj);else if(obj)result.push(obj)})}}return result},base:function(arg){var obj=new Base;if(arg instanceof Object)utility.merge(obj,arg);obj.observer=observable.factory();return obj},blob:function(arg){var obj;try{obj=new Blob([arg],{type:"application/javascript"})}catch(e){if(!global.BlobBuilder)global.BlobBuilder=global.MSBlobBuilder||global.WebKitBlobBuilder||global.MozBlobBuilder;
obj=(new global.BlobBuilder).append(arg).getBlob()}return obj},clearTimers:function(id){if(id===undefined||string.isEmpty(id))throw new Error(label.invalidArguments);if(utility.timer[id]){clearTimeout(utility.timer[id]);delete utility.timer[id]}if(utility.repeating[id]){clearTimeout(utility.repeating[id]);delete utility.repeating[id]}},clone:function(obj,shallow){var clone;if(shallow===true)return obj!==undefined&&obj!==null?json.decode(json.encode(obj)):obj;else if(!obj||regex.primitive.test(typeof obj)||
...
if(value===null||value===undefined)return undefined;else if(value==="true")return true;else if(value==="false")return false;else if(value==="null")return null;else if(value==="undefined")return undefined;else if(value==="")return value;else if(!isNaN(tmp=Number(value)))return tmp;else if(regex.json_wrap.test(value))return json.decode(value,true)||value;else return value},compile:function(reg,pattern,modifiers){reg.compile(pattern,modifiers);return true},curry:function(){var args=array.cast(arguments),
fn=args.shift(),cfn=fn.apply(fn,args);return function(){return cfn.apply(cfn,arguments)}},defer:function(fn,ms,id,repeat){var op;ms=ms||0;repeat=repeat===true;if(id!==undefined)utility.clearTimers(id);else id=utility.uuid(true);op=function(){utility.clearTimers(id);fn()};utility[repeat?"repeating":"timer"][id]=setTimeout(op,ms);return id},delay:function(){if(typeof setImmediate!="undefined")return function(arg){setImmediate(arg)};else if(typeof process!="undefined")return process.nextTick;else return function(arg){setTimeout(arg,
