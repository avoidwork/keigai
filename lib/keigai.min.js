...
function(e){this.reject(e)});kxhr.xhr.send(payload!==null?payload:undefined)});return kxhr},scroll:function(dest,ms){var defer=deferred(),start=client.scrollPos(),t=0;ms=(!isNaN(ms)?ms:250)/100;utility.repeat(function(){var pos=math.bezier(start[0],start[1],dest[0],dest[1],++t/100);window.scrollTo(pos[0],pos[1]);if(t===100){defer.resolve(true);return false}},ms,"scrolling");return defer},scrollPos:function(){return[window.scrollX||0,window.scrollY||0]}};var csv={decode:function(arg,delimiter){delimiter=
delimiter||",";var regex=new RegExp(delimiter+'(?=(?:[^"]|"[^"]*")*$)'),rows=arg.split("\n"),keys=rows.shift().split(delimiter),result=[],nth=rows.length,x=keys.length,i,n,obj,row;for(i=0;i<nth;++i){obj={};row=rows[i].split(regex);for(n=0;n<x;++n)obj[keys[n]]=utility.coerce(row[n]);result.push(obj)}return result},encode:function(arg,delimiter,header){var obj=json.decode(arg,true)||arg,result="";delimiter=delimiter||",";header=header!==false;function prepare(input){var output;if(input instanceof Array){output=
...
defer.resolve(args)},function(e){if(events)self.dispatch("failedBatch",e);defer.reject(e)})}return defer};DataStore.prototype.buildUri=function(key){var parsed=utility.parse(this.uri);return parsed.protocol+"//"+parsed.host+parsed.pathname+(regex.endslash.test(parsed.pathname)?"":"/")+key};DataStore.prototype.clear=function(sync){sync=sync===true;var events=this.events===true;if(!sync){if(events)this.dispatch("beforeClear");array.each(this.lists,function(i){i.teardown(true)});this.autosave=false;
this.callback=null;this.collections=[];this.credentials=null;this.lists=[];this.depth=0;this.events=true;this.expires=null;this.headers={Accept:"application/json"};this.ignore=[];this.key=null;this.keys={};this.leafs=[];this.loaded=false;this.maxDepth=0;this.records=[];this.retrieve=false;this.source=null;this.total=0;this.versions={};this.versioning=true;this.views={};this.uri=null;if(events)this.dispatch("afterClear")}else{this.collections=[];this.keys={};this.loaded=false;this.records=[];this.total=
0;this.views={};array.each(this.lists,function(i){i.refresh()})}return this};DataStore.prototype.crawl=function(arg){var self=this,events=this.events===true,record=arg instanceof Object?arg:this.get(arg),defer=deferred.factory(),deferreds=[],parsed=utility.parse(this.uri||""),clone;if(this.uri===null||record===undefined)throw new Error(label.invalidArguments);if(events)this.dispatch("beforeRetrieve",record);if(record.data instanceof Array){clone=utility.clone(record.data);record.data={};array.each(clone,
function(i){var key=i.replace(/.*\//,""),uri;record.data[key]=store.factory(null,{id:record.key+"-"+key,key:self.key,pointer:self.pointer,source:self.source,ignore:self.ignore.slice(),leafs:self.leafs.slice(),depth:self.depth+1,maxDepth:self.maxDepth,headers:self.headers,retrieve:true});if(i.indexOf("//")===-1)if(i.charAt(0)!=="/")uri=self.buildUri(i);else uri=parsed.protocol+"//"+parsed.host+i;else uri=i;deferreds.push(record.data[key].data.setUri(uri))})}else utility.iterate(record.data,function(v,
k){var uri;if(array.contains(self.ignore,k)||array.contains(self.leafs,k)||self.depth>=self.maxDepth||!(v instanceof Array)&&typeof v!="string"||v.indexOf("//")===-1&&v.charAt(0)!=="/")return;array.add(self.collections,k);record.data[k]=store.factory(null,{id:record.key+"-"+k,key:self.key,source:self.source,ignore:self.ignore.slice(),leafs:self.leafs.slice(),depth:self.depth+1,maxDepth:self.maxDepth,headers:self.headers,retrieve:true});if(!array.contains(self.leafs,k)&&(record.data[k].data.maxDepth===
