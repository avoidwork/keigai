...
result=utility.clone(this.records,true).filter(new Function("rec",cond))}defer.resolve(result)}}return defer};DataStore.prototype.set=function(key,data,batch,overwrite){data=utility.clone(data,true);batch=batch===true;overwrite=overwrite===true;var self=this,events=this.events,defer=deferred.factory(),record=key!==null?this.get(key)||null:data[this.key]?this.get(data[this.key])||null:null,method="POST",parsed=utility.parse(self.uri||""),uri,odata;function patch(overwrite,data,ogdata){var ndata=[];
if(overwrite)array.each(array.keys(ogdata),function(k){if(data[k]===undefined)ndata.push({op:"remove",path:"/"+k})});utility.iterate(data,function(v,k){if(ogdata[k]===undefined)ndata.push({op:"add",path:"/"+k,value:v});else ndata.push({op:"replace",path:"/"+k,value:v})});return ndata}if(typeof data=="string"){if(data.indexOf("//")===-1)if(data.charAt(0)!=="/")uri=this.buildUri(data);else if(self.uri!==null&&regex.root.test(data))uri=parsed.protocol+"//"+parsed.host+data;else uri=data;else uri=data;
key=uri.replace(regex.not_endpoint,"");if(string.isEmpty(key))defer.reject(new Error(label.invalidArguments));else{if(!batch&&events)self.dispatch("beforeSet",{key:key,data:data});client.request(uri,"GET",null,utility.merge({withCredentials:self.credentials},self.headers)).then(function(arg){self.setComplete(record,key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}else{if(!batch&&events)self.dispatch("beforeSet",{key:key,
data:data});if(batch||this.uri===null)this.setComplete(record,key,data,batch,overwrite,defer);else{if(key!==null){method="PUT";uri=this.buildUri(key);odata=utility.clone(data,true);if(client.allows(uri,"patch")){method="PATCH";data=patch(overwrite,data,record.data)}else if(record!==null&&!overwrite)utility.iterate(record.data,function(v,k){data[k]=v})}else uri=this.uri;client.request(uri,method,data,utility.merge({withCredentials:this.credentials},this.headers)).then(function(arg){self.setComplete(record,
key,key===null?self.source?utility.walk(arg,self.source):arg:odata||data,batch,overwrite,defer)},function(e){if(key!==null&&method=="PUT"){method="PATCH";data=patch(overwrite,odata,record.data);client.request(uri,method,data,utility.merge({withCredentials:self.credentials},self.headers)).then(function(){self.setComplete(record,key,odata,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}else{self.dispatch("failedSet",e);defer.reject(e)}})}}return defer};DataStore.prototype.setComplete=
function(record,key,data,batch,overwrite,defer){this.views={};if(key===null)if(this.key!==null&&data[this.key])key=data[this.key].toString();else key=utility.uuid();if(this.key)delete data[this.key];if(record===null){record={index:this.total++,key:key,data:data,indexes:[]};this.indexes.key[key]=record.index;this.records[record.index]=record;if(this.versioning){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}}else{if(this.versioning){if(this.versions[record.key]===undefined){this.versions[record.key]=
lru.factory(VERSIONS);this.versions[record.key].nth=0}this.versions[record.key].set("v"+ ++this.versions[record.key].nth,this.dump([record])[0])}record=this.records[record.index];if(overwrite)record.data={};utility.iterate(data,function(v,k){record.data[k]=v});record=utility.clone(record,true)}this.setIndexes(record);if(!batch){if(this.autosave)this.save();if(this.events)this.dispatch("afterSet",record);array.each(this.lists,function(i){i.refresh()})}defer.resolve(record);return this};DataStore.prototype.setExpires=
function(arg){var id=this.id+"Expire",expires=arg,self=this;if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.invalidArguments);if(this.expires===arg)return;this.expires=arg;utility.clearTimers(id);if(arg===null)return;utility.repeat(function(){if(self.uri===null){self.setExpires(null);return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setIndexes=function(arg){var self=
this,delimter="|";arg.indexes=[];array.each(this.index,function(i){var keys=i.split(delimter),values="";if(self.indexes[i]===undefined)self.indexes[i]={};array.each(keys,function(k,kdx){values+=(kdx>0?delimter:"")+arg.data[k]});if(self.indexes[i][values]===undefined)self.indexes[i][values]=[];if(!array.contains(self.indexes[i][values],arg.index)){self.indexes[i][values].push(arg.index);arg.indexes.push([i,values])}});return this};DataStore.prototype.setUri=function(arg){var defer=deferred.factory(),
parsed;if(arg!==null&&string.isEmpty(arg))defer.reject(new Error(label.invalidArguments));if(arg===null)this.uri=arg;else{parsed=utility.parse(arg);this.uri=parsed.protocol+"//"+parsed.host+parsed.path;if(!string.isEmpty(parsed.auth)&&!this.headers.authorization&&!this.headers.Authorization)this.headers.Authorization="Basic "+btoa(decodeURIComponent(parsed.auth));this.on("expire",function(){this.sync()},"resync",this);cache.expire(this.uri);this.sync().then(function(arg){defer.resolve(arg)},function(e){defer.reject(e)})}return defer};
DataStore.prototype.sort=function(query,create,where){create=create===true||where instanceof Object;var self=this,view=string.toCamelCase(string.explode(query).join(" ")),defer=deferred.factory(),next,worker;next=function(records){if(self.total===0)defer.resolve([]);else if(!create&&self.views[view])defer.resolve(self.views[view]);else if(webWorker){defer.then(function(arg){self.views[view]=arg;return self.views[view]},function(e){utility.error(e)});try{worker=utility.worker(defer);worker.postMessage({cmd:"sort",
indexes:self.indexes,records:records,query:query})}catch(e){webWorker=false;self.views[view]=array.keySort(records,query,"data");defer.resolve(self.views[view])}}else{self.views[view]=array.keySort(records,query,"data");defer.resolve(self.views[view])}};if(!where)next(utility.clone(this.records,true));else this.select(where).then(next,function(e){defer.reject(e)});return defer};DataStore.prototype.storage=function(obj,op,type){var self=this,record=false,mongo=!string.isEmpty(this.mongodb),session=
...
nth=recs.length;if(nth>0){self.records=recs.map(function(r){var rec={key:r._id,index:++i,data:{}};self.indexes.key[rec.key]=rec.index;rec.data=r;delete rec.data._id;self.setIndexes(rec);return rec});self.total=nth}db.close();defer.resolve(self.records)});else if(op==="remove")collection.remove(record?{_id:key}:{},{safe:true},function(e,arg){db.close();if(e)defer.reject(e);else defer.resolve(arg)});else if(op==="set")if(record)collection.update({_id:obj.key},obj.data,{w:1,safe:true,upsert:true},function(e,
arg){db.close();if(e)defer.reject(e);else defer.resolve(arg)});else collection.remove({},{w:1,safe:true},function(e){var deferreds;if(e){db.close();return defer.reject(e)}else{deferreds=[];array.each(self.records,function(i){var data={},defer2=deferred.factory();deferreds.push(defer2);utility.iterate(i.data,function(v,k){data[k]=v});collection.update({_id:i.key},data,{w:1,safe:true,upsert:true},function(e,arg){if(e)defer2.reject(e);else defer2.resolve(arg)})});utility.when(deferreds).then(function(result){db.close();
defer.resolve(result)},function(e){db.close();defer.reject(e)})}});else{db.close();defer.reject(null)}})});else if(op==="get"){result=session?sessionStorage.getItem(key):localStorage.getItem(key);if(result!==null){result=json.decode(result);if(record)self.set(key,result,true).then(function(rec){defer.resolve(rec)},function(e){defer.reject(e)});else{utility.merge(self,result);defer.resolve(self)}}else defer.resolve(self);defer.then(function(){self.loaded=true},function(e){throw e;})}else if(op==="remove"){session?
sessionStorage.removeItem(key):localStorage.removeItem(key);defer.resolve(this)}else if(op==="set"){data=json.encode(record?obj.data:{total:this.total,index:this.index,indexes:this.indexes,records:this.records});session?sessionStorage.setItem(key,data):localStorage.setItem(key,data);defer.resolve(this)}else defer.reject(null)}return defer};DataStore.prototype.sync=function(){var self=this,events=this.events===true,defer=deferred.factory(),success,failure;success=function(arg){var data;if(typeof arg!=
"object")return failure(new Error(label.expectedObject));if(self.source!==null)arg=utility.walk(arg,self.source);if(arg instanceof Array)data=arg;else data=[arg];self.batch("set",data,true).then(function(arg){if(events)self.dispatch("afterSync",arg);defer.resolve(arg)},failure)};failure=function(e){if(events)self.dispatch("failedSync",e);defer.reject(e)};if(this.uri===null||string.isEmpty(this.uri))defer.reject(new Error(label.invalidArguments));else{if(events)this.dispatch("beforeSync",this.uri);
if(this.callback!==null)client.jsonp(this.uri,{callback:this.callback}).then(success,failure);else client.request(this.uri,"GET",null,utility.merge({withCredentials:this.credentials},this.headers)).then(success,failure)}return defer};DataStore.prototype.teardown=function(){var uri=this.uri,id;if(uri!==null){cache.expire(uri,true);id=this.id+"DataExpire";utility.clearTimers(id);array.each(this.records,function(i){var recordUri=uri+"/"+i.key;cache.expire(recordUri,true)})}array.each(this.lists,function(i){i.teardown(true)});
this.clear(true);this.dispatch("afterTeardown");return this};DataStore.prototype.undo=function(key,version){var record=this.get(key),defer=deferred.factory(),versions=this.versions[record.key],previous;if(record===undefined)defer.reject(new Error(label.invalidArguments));else if(versions){previous=versions.get(version||versions.first);if(previous===undefined)defer.reject(label.datastoreNoPrevVersion);else this.set(key,previous).then(function(arg){defer.resolve(arg)},function(e){defer.reject(e)})}else defer.reject(label.datastoreNoPrevVersion);
...
delete utility.repeating[id]}},clone:function(obj,shallow){var clone;if(shallow===true)return obj!==undefined&&obj!==null?json.decode(json.encode(obj)):obj;else if(!obj||regex.primitive.test(typeof obj)||obj instanceof RegExp)return obj;else if(obj instanceof Array)return obj.slice();else if(!server&&!client.ie&&obj instanceof Document)return xml.decode(xml.encode(obj));else if(typeof obj.__proto__!="undefined")return utility.extend(obj.__proto__,obj);else if(obj instanceof Object){clone=json.encode(obj,
true);if(clone!==undefined){clone=json.decode(clone);utility.iterate(obj,function(v,k){if(typeof v=="function")clone[k]=v})}else clone=obj;return clone}else return obj},coerce:function(value){var tmp;if(value===null||value===undefined)return undefined;else if(value==="true")return true;else if(value==="false")return false;else if(value==="null")return null;else if(value==="undefined")return undefined;else if(value==="")return value;else if(!isNaN(tmp=Number(value)))return tmp;else if(regex.json_wrap.test(value))return json.decode(value,
...
uuid:utility.uuid,walk:utility.walk,when:utility.when},version:"0.9.0"}}();if(typeof exports!="undefined")module.exports=lib;else if(typeof define=="function")define(function(){return lib});else global.keigai=lib})(this);
