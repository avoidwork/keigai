...
'rec.data["'+clauses[0][0]+'"] === "'+clauses[0][1]+'"';cond+=" );";result=ev.data.records.filter(new Function("rec",cond))}else if(cmd==="sort")result=array.keySort(ev.data.records,ev.data.query,"data");postMessage(result)}};function DataStore(){this.autosave=false;this.callback=null;this.credentials=null;this.lists=[];this.events=true;this.expires=null;this.headers={Accept:"application/json"};this.ignore=[];this.index=[];this.indexes={key:{}};this.key=null;this.loaded=false;this.mongodb="";this.observer=
observable.factory();this.records=[];this.source=null;this.total=0;this.versions={};this.versioning=true;this.views={};this.uri=null}DataStore.prototype=base.factory();DataStore.prototype.constructor=DataStore;DataStore.prototype.batch=function(type,data,sync){sync=sync===true;var self=this,events=this.events,defer=deferred.factory(),deferreds=[];if(!regex.set_del.test(type)||sync&&regex.del.test(type)||typeof data!="object")defer.reject(new Error(label.invalidArguments));else{if(events)this.dispatch("beforeBatch",
...
this.headers)).then(function(){self.delComplete(record,reindex,batch,defer)},function(e){self.dispatch("failedDelete",e);defer.reject(e)})}return defer};DataStore.prototype.delComplete=function(record,reindex,batch,defer){delete this.indexes.key[record.key];delete this.versions[record.key];array.remove(this.records,record.index);this.total--;this.views={};if(!batch){if(reindex)this.reindex();if(this.autosave)this.purge(record.key);if(this.events)this.dispatch("afterDelete",record);array.each(this.lists,
function(i){i.refresh()})}return defer.resolve(record.key)};DataStore.prototype.dump=function(args,fields){args=args||this.records;var self=this,custom=fields instanceof Array&&fields.length>0,key=this.key!==null,fn;if(custom)fn=function(i){var record={};array.each(fields,function(f){record[f]=f===self.key?i.key:utility.clone(i.data[f],true)});return record};else fn=function(i){var record={};if(key)record[self.key]=i.key;utility.iterate(i.data,function(v,k){record[k]=utility.clone(v,true)});return record};
return args.map(fn)};DataStore.prototype.get=function(record,offset){var type=typeof record,self=this,result;if(type==="undefined")result=this.records;else if(type==="string")if(record.indexOf(",")===-1)result=this.records[this.indexes.key[record]];else result=string.explode(record).map(function(i){if(!isNaN(i))return self.records[parseInt(i,10)];else return self.records[self.indexes.key[i]]});else if(type==="number")if(isNaN(offset))result=this.records[parseInt(record,10)];else result=array.limit(this.records,
parseInt(record,10),parseInt(offset,10));return result},DataStore.prototype.join=function(arg,field,join){join=join||"inner";var self=this,defer=deferred.factory(),results=[],deferreds=[],key=field===this.key,keys=array.merge(array.cast(this.records[0].data,true),array.cast(arg.records[0].data,true)),fn;if(join==="inner")fn=function(i){var where={},record=utility.clone(i.data,true),defer=deferred.factory();where[field]=key?i.key:record[field];arg.select(where).then(function(match){if(match.length>
...
function(arg){return this.storage(arg||this,"remove")};DataStore.prototype.reindex=function(){var self=this,i=-1,tmp=[];this.views={};this.indexes={};if(this.total>0)array.each(this.records,function(record){if(record!==undefined){tmp[++i]=record;record.index=i;self.indexes.key[record.key]=i;self.setIndexes(record)}});this.records=tmp;return this};DataStore.prototype.restore=function(arg){return this.storage(arg||this,"get")};DataStore.prototype.save=function(arg){return this.storage(arg||this,"set")};
DataStore.prototype.select=function(where){var defer=deferred.factory(),clauses,cond,functions,worker;if(!(where instanceof Object))defer.reject(new Error(label.invalidArguments));else if(webWorker){functions=[];utility.iterate(where,function(v,k){if(typeof v=="function"){this[k]=v.toString();functions.push(k)}});try{worker=utility.worker(defer);worker.postMessage({cmd:"select",indexes:this.indexes,records:this.records,where:json.encode(where),functions:functions})}catch(e){webWorker=false;this.select(where).then(function(arg){defer.resolve(arg)},
function(e){defer.reject(e)})}}else{clauses=array.fromObject(where);cond="return ( ";if(clauses.length>1)array.each(clauses,function(i,idx){var b1="( ";if(idx>0)b1=" && ( ";if(i[1]instanceof Function)cond+=b1+i[1].toString()+'( rec.data["'+i[0]+'"] ) )';else if(!isNaN(i[1]))cond+=b1+'rec.data["'+i[0]+'"] === '+i[1]+" )";else cond+=b1+'rec.data["'+i[0]+'"] === "'+i[1]+'" )'});else if(clauses[0][1]instanceof Function)cond+=clauses[0][1].toString()+'( rec.data["'+clauses[0][0]+'"] )';else if(!isNaN(clauses[0][1]))cond+=
'rec.data["'+clauses[0][0]+'"] === '+clauses[0][1];else cond+='rec.data["'+clauses[0][0]+'"] === "'+clauses[0][1]+'"';cond+=" );";defer.resolve(utility.clone(this.records,true).filter(new Function("rec",cond)))}return defer};DataStore.prototype.set=function(key,data,batch,overwrite){data=utility.clone(data,true);batch=batch===true;overwrite=overwrite===true;var self=this,events=this.events,defer=deferred.factory(),record=key!==null?this.get(key)||null:data[this.key]?this.get(data[this.key])||null:
null,method="POST",parsed=utility.parse(self.uri||""),uri;if(typeof data=="string"){if(data.indexOf("//")===-1)if(data.charAt(0)!=="/")uri=this.buildUri(data);else if(self.uri!==null&&regex.root.test(data))uri=parsed.protocol+"//"+parsed.host+data;else uri=data;else uri=data;key=uri.replace(regex.not_endpoint,"");if(string.isEmpty(key))defer.reject(new Error(label.invalidArguments));else{if(!batch&&events)self.dispatch("beforeSet",{key:key,data:data});client.request(uri,"GET",null,utility.merge({withCredentials:self.credentials},
self.headers)).then(function(arg){self.setComplete(record,key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}else{if(!batch&&events)self.dispatch("beforeSet",{key:key,data:data});if(batch||this.uri===null)this.setComplete(record,key,data,batch,overwrite,defer);else{if(key!==null){method="PUT";uri=this.buildUri(key);if(client.allows(uri,"patch"))method="PATCH";else if(record!==null&&!overwrite)utility.iterate(record.data,
function(v,k){data[k]=v})}else uri=this.uri;client.request(uri,method,data,utility.merge({withCredentials:this.credentials},this.headers)).then(function(arg){self.setComplete(record,key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}return defer};DataStore.prototype.setComplete=function(record,key,data,batch,overwrite,defer){this.views={};if(!key)if(this.key!==null&&data[this.key])key=data[this.key].toString();else key=
utility.uuid();if(this.key)delete data[this.key];if(record===null){record={index:this.total++,key:key,data:data};this.indexes.key[key]=record.index;this.records[record.index]=record;if(this.versioning){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}}else{if(this.versioning){if(this.versions[record.key]===undefined){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}this.versions[record.key].set("v"+ ++this.versions[record.key].nth,this.dump([record])[0])}if(overwrite)record.data=
{};utility.iterate(data,function(v,k){record.data[k]=v})}this.setIndexes(record);if(!batch){if(this.autosave)this.save(record);if(this.events)this.dispatch("afterSet",record);array.each(this.lists,function(i){i.refresh()})}defer.resolve(record);return this};DataStore.prototype.setExpires=function(arg){var id=this.id+"Expire",expires=arg,self=this;if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.invalidArguments);if(this.expires===arg)return;this.expires=arg;
utility.clearTimers(id);if(arg===null)return;utility.repeat(function(){if(self.uri===null){self.setExpires(null);return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setIndexes=function(arg){var self=this,delimter="|";array.each(this.index,function(i){var keys=i.split(delimter),values="";if(self.indexes[i]===undefined)self.indexes[i]={};array.each(keys,function(k,kdx){values+=(kdx>0?delimter:
"")+arg.data[k]});if(self.indexes[i][values]===undefined)self.indexes[i][values]=[];self.indexes[i][values].push(arg.index)});return this};DataStore.prototype.setUri=function(arg){var defer=deferred.factory(),parsed;if(arg!==null&&string.isEmpty(arg))defer.reject(new Error(label.invalidArguments));if(arg===null)this.uri=arg;else{parsed=utility.parse(arg);this.uri=parsed.protocol+"//"+parsed.host+parsed.path;if(!string.isEmpty(parsed.auth)&&!this.headers.authorization&&!this.headers.Authorization)this.headers.Authorization=
