...
if(!batch){if(this.autosave)this.save(record);if(this.events)this.dispatch("afterSet",record);array.each(this.lists,function(i){i.refresh()})}defer.resolve(record);return this};DataStore.prototype.setExpires=function(arg){var id=this.id+"Expire",expires=arg,self=this;if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.invalidArguments);if(this.expires===arg)return;this.expires=arg;utility.clearTimers(id);if(arg===null)return;utility.repeat(function(){if(self.uri===
null){self.setExpires(null);return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setIndexes=function(arg){var self=this,delimter="|";arg.indexes=[];array.each(this.index,function(i){var keys=i.split(delimter),values="";if(self.indexes[i]===undefined)self.indexes[i]={};array.each(keys,function(k,kdx){values+=(kdx>0?delimter:"")+arg.data[k]});if(self.indexes[i][values]===undefined)self.indexes[i][values]=
[];self.indexes[i][values].push(arg.index);arg.indexes.push([i,values])});return this};DataStore.prototype.setUri=function(arg){var defer=deferred.factory(),parsed;if(arg!==null&&string.isEmpty(arg))defer.reject(new Error(label.invalidArguments));if(arg===null)this.uri=arg;else{parsed=utility.parse(arg);this.uri=parsed.protocol+"//"+parsed.host+parsed.path;if(!string.isEmpty(parsed.auth)&&!this.headers.authorization&&!this.headers.Authorization)this.headers.Authorization="Basic "+btoa(decodeURIComponent(parsed.auth));
...
else if(webWorker){defer.then(function(arg){self.views[view]=arg;return self.views[view]},function(e){utility.error(e)});try{worker=utility.worker(defer);worker.postMessage({cmd:"sort",indexes:self.indexes,records:records,query:query})}catch(e){webWorker=false;self.views[view]=array.keySort(utility.clone(records),query,"data");defer.resolve(self.views[view])}}else{self.views[view]=array.keySort(utility.clone(records),query,"data");defer.resolve(self.views[view])}};if(!where)next(this.records);else this.select(where).then(next,
...
obj=(new global.BlobBuilder).append(arg).getBlob()}return obj},clearTimers:function(id){if(id===undefined||string.isEmpty(id))throw new Error(label.invalidArguments);if(utility.timer[id]){clearTimeout(utility.timer[id]);delete utility.timer[id]}if(utility.repeating[id]){clearTimeout(utility.repeating[id]);delete utility.repeating[id]}},clone:function(obj,shallow){var clone;if(shallow===true)return json.decode(json.encode(obj));else if(!obj||regex.primitive.test(typeof obj)||obj instanceof RegExp)return obj;
...
else if(value==="true")return true;else if(value==="false")return false;else if(value==="null")return null;else if(value==="undefined")return undefined;else if(value==="")return value;else if(!isNaN(tmp=Number(value)))return tmp;else if(regex.json_wrap.test(value))return json.decode(value,true)||value;else return value},compile:function(reg,pattern,modifiers){reg.compile(pattern,modifiers);return true},curry:function(){var args=array.cast(arguments),fn=args.shift(),cfn=fn.apply(fn,args);return function(){return cfn.apply(cfn,
arguments)}},defer:function(fn,ms,id,repeat){var op;ms=ms||0;repeat=repeat===true;if(id!==undefined)utility.clearTimers(id);else id=utility.uuid(true);op=function(){utility.clearTimers(id);fn()};utility[repeat?"repeating":"timer"][id]=setTimeout(op,ms);return id},delay:function(){if(typeof setImmediate!="undefined")return function(arg){setImmediate(arg)};else if(typeof process!="undefined")return process.nextTick;else return function(arg){setTimeout(arg,0)}}(),dom:function(arg){var result;if(!regex.selector_complex.test(arg))if(regex.hash.test(arg))result=
