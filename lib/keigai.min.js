...
this.headers)).then(function(){self.delComplete(record,reindex,batch,defer)},function(e){self.dispatch("failedDelete",e);defer.reject(e)})}return defer};DataStore.prototype.delComplete=function(record,reindex,batch,defer){var self=this;delete this.indexes.key[record.key];delete this.versions[record.key];array.remove(this.records,record.index);this.total--;this.views={};if(!batch){if(reindex)this.reindex();else array.each(record.indexes,function(i){array.remove(self.indexes[i[0]][i[1]],record.index);
if(self.indexes[i[0]][i[1]].length===0)delete self.indexes[i[0]][i[1]]});if(this.autosave)this.purge(record.key);if(this.events)this.dispatch("afterDelete",record);array.each(this.lists,function(i){i.refresh()})}return defer.resolve(record.key)};DataStore.prototype.dump=function(args,fields){args=args||this.records;var self=this,custom=fields instanceof Array&&fields.length>0,key=this.key!==null,fn;if(custom)fn=function(i){var record={};array.each(fields,function(f){record[f]=f===self.key?i.key:utility.clone(i.data[f],
true)});return record};else fn=function(i){var record={};if(key)record[self.key]=i.key;utility.iterate(i.data,function(v,k){record[k]=utility.clone(v,true)});return record};return args.map(fn)};DataStore.prototype.get=function(record,offset){var type=typeof record,self=this,result;if(type==="undefined")result=this.records;else if(type==="string")if(record.indexOf(",")===-1)result=this.records[this.indexes.key[record]];else result=string.explode(record).map(function(i){if(!isNaN(i))return self.records[parseInt(i,
10)];else return self.records[self.indexes.key[i]]});else if(type==="number")if(isNaN(offset))result=this.records[parseInt(record,10)];else result=array.limit(this.records,parseInt(record,10),parseInt(offset,10));return result},DataStore.prototype.join=function(arg,field,join){join=join||"inner";var self=this,defer=deferred.factory(),results=[],deferreds=[],key=field===this.key,keys=array.merge(array.cast(this.records[0].data,true),array.cast(arg.records[0].data,true)),fn;if(join==="inner")fn=function(i){var where=
...
lru.factory(VERSIONS);this.versions[record.key].nth=0}this.versions[record.key].set("v"+ ++this.versions[record.key].nth,this.dump([record])[0])}if(overwrite)record.data={};utility.iterate(data,function(v,k){record.data[k]=v})}this.setIndexes(record);if(!batch){if(this.autosave)this.save(record);if(this.events)this.dispatch("afterSet",record);array.each(this.lists,function(i){i.refresh()})}defer.resolve(record);return this};DataStore.prototype.setExpires=function(arg){var id=this.id+"Expire",expires=
arg,self=this;if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.invalidArguments);if(this.expires===arg)return;this.expires=arg;utility.clearTimers(id);if(arg===null)return;utility.repeat(function(){if(self.uri===null){self.setExpires(null);return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setIndexes=function(arg){var self=this,delimter="|";arg.indexes=
[];array.each(this.index,function(i){var keys=i.split(delimter),values="";if(self.indexes[i]===undefined)self.indexes[i]={};array.each(keys,function(k,kdx){values+=(kdx>0?delimter:"")+arg.data[k]});if(self.indexes[i][values]===undefined)self.indexes[i][values]=[];self.indexes[i][values].push(arg.index);arg.indexes.push([i,values])});return this};DataStore.prototype.setUri=function(arg){var defer=deferred.factory(),parsed;if(arg!==null&&string.isEmpty(arg))defer.reject(new Error(label.invalidArguments));
...
deferred.factory(),data,key,result;if(!regex.number_string_object.test(typeof obj)||!regex.get_remove_set.test(op))defer.reject(new Error(label.invalidArguments));else{record=regex.number_string.test(typeof obj)||obj.hasOwnProperty("data");if(op!=="remove"){if(record&&!(obj instanceof Object))obj=this.get(obj);key=record?obj.key:obj.id}else if(op==="remove"&&record)key=obj.key||obj;if(mongo)mongodb.connect(this.mongodb,function(e,db){if(e){if(db)db.close();return defer.reject(e)}db.collection(self.id,
function(e,collection){if(e){db.close();return defer.reject(e)}if(op==="get")if(record)collection.find({_id:obj.key}).limit(1).toArray(function(e,recs){db.close();if(e)defer.reject(e);else if(recs.length===0)defer.resolve(null);else{delete recs[0]._id;self.set(key,recs[0],true).then(function(rec){defer.resolve(rec)},function(e){defer.reject(e)})}});else collection.find({}).toArray(function(e,recs){var i,nth;if(e){db.close();return defer.reject(e)}i=-1;nth=recs.length;if(nth>0){self.records=recs.map(function(r){var rec=
{key:r._id,index:++i,data:{}};self.indexes.key[rec.key]=rec.index;rec.data=r;delete rec.data._id;self.setIndexes(rec);return rec});self.total=nth}db.close();defer.resolve(self.records)});else if(op==="remove")collection.remove(record?{_id:key}:{},{safe:true},function(e,arg){db.close();if(e)defer.reject(e);else defer.resolve(arg)});else if(op==="set")if(record)collection.update({_id:obj.key},obj.data,{w:1,safe:true,upsert:true},function(e,arg){db.close();if(e)defer.reject(e);else defer.resolve(arg)});
else collection.remove({},{w:1,safe:true},function(e){var deferreds;if(e){db.close();return defer.reject(e)}else{deferreds=[];array.each(self.records,function(i){var data={},defer2=deferred.factory();deferreds.push(defer2);utility.iterate(i.data,function(v,k){data[k]=v});collection.update({_id:i.key},data,{w:1,safe:true,upsert:true},function(e,arg){if(e)defer2.reject(e);else defer2.resolve(arg)})});utility.when(deferreds).then(function(result){db.close();defer.resolve(result)},function(e){db.close();
...
walk:utility.walk,when:utility.when},version:"0.8.0"}}();if(typeof exports!="undefined")module.exports=lib;else if(typeof define=="function")define(function(){return lib});else global.keigai=lib})(this);
