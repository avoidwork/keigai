...
"DELETE",null,utility.merge({withCredentials:this.credentials},this.headers)).then(function(){self.delComplete(record,reindex,batch,defer)},function(e){self.dispatch("failedDelete",e);defer.reject(e)})}return defer};DataStore.prototype.delComplete=function(record,reindex,batch,defer){delete this.keys[record.key];delete this.versions[record.key];array.remove(this.records,record.index);this.total--;this.views={};array.each(this.collections,function(i){record.data[i].teardown()});if(!batch){if(reindex)this.reindex();
...
key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}else{if(!batch&&events)self.dispatch("beforeSet",{key:key,data:data});if(batch||this.uri===null)this.setComplete(record,key,data,batch,overwrite,defer);else{if(key!==null){method="PUT";uri=this.buildUri(key);if(client.allows(uri,"patch"))method="PATCH";else if(record!==null&&!overwrite)utility.iterate(record.data,function(v,k){if(!array.contains(self.collections,k)&&
!data[k])data[k]=v})}else uri=this.uri;client.request(uri,method,data,utility.merge({withCredentials:this.credentials},this.headers)).then(function(arg){self.setComplete(record,key,self.source?utility.walk(arg,self.source):arg,batch,overwrite,defer)},function(e){self.dispatch("failedSet",e);defer.reject(e)})}}return defer};DataStore.prototype.setComplete=function(record,key,data,batch,overwrite,defer){var self=this,deferreds=[];this.views={};if(!key)if(this.key!==null&&data[this.key])key=data[this.key].toString();
else key=utility.uuid();if(this.key)delete data[this.key];if(record===null){record={index:this.total++,key:key,data:data};this.keys[key]=record.index;this.records[record.index]=record;if(this.versioning){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}if(this.retrieve)deferreds.push(this.crawl(record))}else{if(this.versioning){if(this.versions[record.key]===undefined){this.versions[record.key]=lru.factory(VERSIONS);this.versions[record.key].nth=0}this.versions[record.key].set("v"+
++this.versions[record.key].nth,this.dump([record])[0])}if(overwrite){array.each(this.collections,function(i){if(record.data[i]&&typeof record.data[i].teardown=="function")record.data[i].teardown()});record.data={}}utility.iterate(data,function(v,k){if(!array.contains(self.collections,k))record.data[k]=v;else if(typeof v=="string")deferreds.push(record.data[k].setUri(record.data[k].uri+"/"+v,true));else deferreds.push(record.data[k].batch("set",v,true))})}if(!batch){if(this.autosave)this.save(record);
if(this.events)this.dispatch("afterSet",record);array.each(this.lists,function(i){i.refresh()})}if(deferreds.length===0)defer.resolve(record);else utility.when(deferreds).then(function(){defer.resolve(record)});return this};DataStore.prototype.setExpires=function(arg){if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.invalidArguments);if(this.expires===arg)return;this.expires=arg;var id=this.id+"Expire",expires=arg,self=this;utility.clearTimers(id);if(arg===null)return;
utility.repeat(function(){if(self.uri===null){self.setExpires(null);return false}self.dispatch("beforeExpire");cache.expire(self.uri);self.dispatch("expire");self.dispatch("afterExpire")},expires,id,false)};DataStore.prototype.setUri=function(arg){var defer=deferred.factory(),parsed,uri;if(arg!==null&&string.isEmpty(arg))throw new Error(label.invalidArguments);parsed=utility.parse(arg);uri=parsed.href;if(array.keys(parsed.query).length>0){uri=uri.replace(/\?.*/,"?");utility.iterate(parsed.query,function(v,
k){if(!(v instanceof Array))uri+="&"+k+"="+encodeURIComponent(v);else array.each(v,function(i){uri+="&"+k+"="+encodeURIComponent(i)})});uri=uri.replace("?&","?")}this.uri=uri;if(this.uri!==null){this.on("expire",function(){this.sync()},"resync",this);cache.expire(this.uri);this.sync().then(function(arg){defer.resolve(arg)},function(e){defer.reject(e)})}return defer};DataStore.prototype.sort=function(query,create,where){create=create===true||where instanceof Object;var self=this,view=string.toCamelCase(string.explode(query).join(" ")),
...
obj.hasOwnProperty("data");if(op!=="remove"){if(record&&!(obj instanceof Object))obj=this.get(obj);key=record?obj.key:obj.id}else if(op==="remove"&&record)key=obj.key||obj;if(mongo)mongodb.connect(this.mongodb,function(e,db){if(e){if(db)db.close();return defer.reject(e)}db.collection(self.id,function(e,collection){if(e){db.close();return defer.reject(e)}if(op==="get")if(record)collection.find({_id:obj.key}).limit(1).toArray(function(e,recs){db.close();if(e)defer.reject(e);else if(recs.length===0)defer.resolve(null);
else{delete recs[0]._id;self.set(key,recs[0],true).then(function(rec){defer.resolve(rec)},function(e){defer.reject(e)})}});else collection.find({}).toArray(function(e,recs){var i,nth;if(e){db.close();return defer.reject(e)}i=-1;nth=recs.length;if(nth>0){self.records=recs.map(function(r){var rec={key:r._id,index:++i,data:{}};self.keys[rec.key]=rec.index;rec.data=r;delete rec.data._id;return rec});self.total=nth}db.close();defer.resolve(self.records)});else if(op==="remove")collection.remove(record?
...
"")},toFunction:function(obj){var args=string.trim(obj.replace(/^.*\(/,"").replace(/[\t|\r|\n|\"|\']+/g,"").replace(/\).*/,"")),body=string.trim(obj.replace(/^.*\{/,"").replace(/\}$/,""));return Function.apply(Function,string.explode(args).concat([body]))},trim:function(obj){return obj.replace(/^(\s+|\t+|\n+)|(\s+|\t+|\n+)$/g,"")},unCamelCase:function(obj){return string.trim(obj.replace(/([A-Z])/g," $1").toLowerCase())},uncapitalize:function(obj){obj=string.trim(obj);return obj.charAt(0).toLowerCase()+
obj.slice(1)},unhyphenate:function(obj,caps){if(caps!==true)return string.explode(obj,"-").join(" ");else return string.explode(obj,"-").map(function(i){return string.capitalize(i)}).join(" ")}};var utility={timer:{},repeating:{},$:function(arg){var result;if(!arg);else if(regex.html.test(arg))result=[element.create(arg)];else{arg=string.trim(arg);if(arg.indexOf(",")===-1){result=utility.dom(arg);if(result){if(isNaN(result.length))result=[result]}else result=[]}else{result=[];array.each(string.explode(arg),
